...
sub _construct_query {
    ...
    return
'select study.internal_id, aliquots.sample_internal_id, aliquots.receptacle_internal_id, sample.name,sample.accession_number, sample.common_name, study.accession_number, sample.supplier_name from current_studies as study 
...
sub _build_libraries_metadata {
    ...
    for my $library_aliquot (@$library_aliquots) {
        my $library_metadata_container = UpdatePipeline::LibraryMetaData->new(
            ...
            study_accession_number  => @$library_aliquot[6],
            ...
            supplier_name           => @$library_aliquot[7],
        );
